...
class Pacman:
    def __init__(self, row, col, image):
        ...
        self.col = col
        self.score = 0
        self.image = pygame.transform.scale(pygame.image.load(image), (20, 20))  # Ubah ukuran gambar Pac-Man
...
class Ghost:
    def __init__(self, row, col, color, behavior):
        self.row = row
        self.col = col
        self.color = color
    ...
    def chase(self, grid, pacman):
        target = (pacman.row, pacman.col)
        self.find_path(grid, target)

    def scatter(self, grid):
        # Define target positions for each ghost based on their behavior
        target_positions = {
            "blinky": (0, GRID_WIDTH - 1),  # Top right corner
            "pinky": (0, 0),  # Top left corner
            "inky": (GRID_HEIGHT - 1, GRID_WIDTH - 1),  # Bottom right corner
            "clyde": (GRID_HEIGHT - 1, 0)  # Bottom left corner
        }
        target = target_positions[self.color]
        self.find_path(grid, target)

    def frightened(self, grid, pacman):
        # Scatter behavior can be used when ghosts are frightened
        self.scatter(grid)

    def return_home(self, grid):
        # Define target positions for each ghost to return to their home position
        home_positions = {
            "blinky": (GRID_HEIGHT // 2 - 3, GRID_WIDTH // 2),
            "pinky": (GRID_HEIGHT // 2 - 3, GRID_WIDTH // 2 + 1),
            "inky": (GRID_HEIGHT // 2 + 2, GRID_WIDTH // 2),
            "clyde": (GRID_HEIGHT // 2 + 2, GRID_WIDTH // 2 + 1)
        }
        target = home_positions[self.color]
        self.find_path(grid, target)
...
def generate_ghost_positions(pacman_row, pacman_col):
    ...
    while len(ghost_positions) < 4:  # Jumlah hantu
...
def main():
    ...
    pacman_image = os.path.join("assets/player_images", "1.png")
    pacman = Pacman(13, 15, pacman_image)

    # Inisialisasi hantu
    ghost_images = [os.path.join("assets/ghost_images", "red.png"),
                    os.path.join("assets/ghost_images", "pink.png"),
                    os.path.join("assets/ghost_images", "blue.png"),
                    os.path.join("assets/ghost_images", "orange.png")]
    ghost_behaviors = ["chase", "scatter", "frightened", "return_home"]  # Define behavior for each ghost
    ghost_positions = generate_ghost_positions(pacman.row, pacman.col)
    ghosts = [Ghost(row, col, ghost_images[i], ghost_behaviors[i]) for i, (row, col) in enumerate(ghost_positions)]
    ...
    # Main loop
    running = True
    pacman_automatic_movement_timer = 300  # Waktu dalam frame (300 frames = 5 detik)
    while running:
        ...
        for ghost in ghosts:
            ghost.move(grid)
            ghost.chase(grid, pacman)  # Berikan objek pacman sebagai argumen
        ...
        # Gambar Pac-Man
        screen.blit(pacman.image, ((MARGIN + CELL_SIZE) * pacman.col + MARGIN, (MARGIN + CELL_SIZE) * pacman.row + MARGIN))

        # Gambar hantu
        for ghost in ghosts:
            ghost_image = pygame.transform.scale(pygame.image.load(ghost.color), (20, 20))  # Ubah ukuran gambar hantu
            screen.blit(ghost_image, ((MARGIN + CELL_SIZE) * ghost.col + MARGIN, (MARGIN + CELL_SIZE) * ghost.row + MARGIN))

        # Update the display
        pygame.display.flip()

        # Pergerakan otomatis Pac-Man setelah 5 detik
        if pacman_automatic_movement_timer > 0:
            pacman_automatic_movement_timer -= 1
        else:
            # Implementasi pergerakan otomatis di sini
            pass
